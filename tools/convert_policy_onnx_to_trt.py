#!/usr/bin/env python3
"""Convert the project policy ONNX file into a TensorRT engine.

This script is aligned with the current rl_controller contract:
...
It first validates the ONNX graph when the Python onnx package is available,
then calls trtexec to build a TensorRT engine, and finally checks the engine
contract when TensorRT Python bindings are available.
"""
...
DEFAULT_ONNX = REPO_ROOT / "policy.onnx"
DEFAULT_ENGINE = REPO_ROOT / "policy.engine"
DEFAULT_TRTEXEC = "/usr/src/tensorrt/bin/trtexec"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Convert the project policy ONNX file to a TensorRT engine."
    )
    parser.add_argument(
        "--input",
        default=str(DEFAULT_ONNX),
        help="Input ONNX file. Default points to the repo root policy.onnx.",
    )
    parser.add_argument(
        "--output",
        default=str(DEFAULT_ENGINE),
        help="Output TensorRT engine file. Default is policy.engine in the repo root.",
    )
    parser.add_argument(
        "--trtexec",
    ...
    parser.add_argument(
        ...
        type=int,
        default=270,
        help="Expected flattened observation dimension. Current runtime expects 270.",
    )
    parser.add_argument(
        "--action-dim",
        type=int,
        default=12,
        help="Expected action dimension. Current runtime expects 12.",
    )
    parser.add_argument(
        "--workspace-mib",
        type=int,
...
def load_onnx_optional(input_path: Path, expected_input: str, expected_output: str, obs_dim: int, action_dim: int) -> None:
    try:
    ...
    if len(inputs) != 1 or len(outputs) != 1:
        fail(f"ONNX graph must have exactly one input and one output, got {len(inputs)} input(s) and {len(outputs)} output(s).")

    input_info = inputs[0]
    output_info = outputs[0]

    if input_info.name != expected_input:
        fail(f"ONNX input name is '{input_info.name}', expected '{expected_input}'. Re-export with the current tools script.")
    if output_info.name != expected_output:
        fail(f"ONNX output name is '{output_info.name}', expected '{expected_output}'. Re-export with the current tools script.")
...
def validate_engine_optional(output_path: Path, obs_dim: int, action_dim: int) -> None:
    ...
    if hasattr(engine, "num_bindings"):
        ...
        warn("TensorRT Python API does not expose legacy binding inspection helpers; skipping detailed validation.")
...
def main() -> int:
    ...
    print("[OK] TensorRT engine export finished")
